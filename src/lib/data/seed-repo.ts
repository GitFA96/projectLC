...
/**
 * Milestone-1 repository: all data from seed JSON, validated against the SAME
 * zod schemas the import parsers (M2) will emit. Validation throws loudly at
 * module init so shape drift is caught on dev boot, not in a component.
 */
...
const guild = parse("guild.json", seedGuildSchema, guildJson);
const roster = parse("roster.json", seedRosterSchema, rosterJson);
const items = parse("items.json", seedItemsSchema, itemsJson);
const gearSets = parse("gear-sets.json", seedGearSetsSchema, gearSetsJson);
const raidSessions = parse("raid-sessions.json", seedRaidSessionsSchema, raidSessionsJson);
const lootAwards = parse("loot-awards.json", seedLootAwardsSchema, lootAwardsJson);
...
/* Referential integrity — hard errors, these always indicate a broken seed file. */
for (const set of gearSets) {
  if (!charactersById.has(set.characterId)) fail("gear-sets.json", new Error(`unknown characterId ${set.characterId} in ${set.id}`));
}
for (const award of lootAwards) {
  if (!sessionsById.has(award.raidSessionId)) fail("loot-awards.json", new Error(`unknown raidSessionId ${award.raidSessionId} in ${award.id}`));
  if (award.characterId !== null && !charactersById.has(award.characterId)) fail("loot-awards.json", new Error(`unknown characterId ${award.characterId} in ${award.id}`));
}
