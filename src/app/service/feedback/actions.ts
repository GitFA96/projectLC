...
/*
 * `submitFeedback` is deliberately the one action here with no check at all.
 * It is the only thing in this app a non-officer can write with, and gating it
 * would close the channel that tells us the gates are wrong. Triage is the
 * opposite: bug reports are about the *application*, so they are service work
 * and answer to the app admin, never to a guild role — no guild may grant
 * somebody the right to read another guild's bug reports.
 */
