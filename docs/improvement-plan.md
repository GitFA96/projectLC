...
| D5 | `npm audit` step | done | `npm audit --omit=dev` in CI, `continue-on-error` on purpose — an advisory against a transitive dev dependency must not be able to stop a raid-night fix from shipping, and Dependabot already opens the PRs. **It is not clean today:** four high-severity libvips CVEs reach the tree through `sharp@0.34.5`, which `next@16.2.9` pulls in, and the fix is a Next minor bump. Nothing in this app calls `next/image` — `item-icon.tsx` uses a plain `<img>` on purpose, with the lint disable to say so — so the optimizer that links libvips never runs. That is a reason it is not urgent, not a reason it is fine, and the bump is a decision for the maintainer rather than a step in this item |
